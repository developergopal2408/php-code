                    ...
                    <table id="od" class="table table-condensed table-bordered display">
                        ...
                        <tbody class="text-sm">
                            ...
                            while ($row = sqlsrv_fetch_array($result)) {
                                ...
                                $ldate = $row['date'];
                                $sql = sqlsrv_query($connection, "select * from member where officeid = '$id' and memberid = '$mid'");
                                $mrow = sqlsrv_fetch_array($sql);
                                $mname = $mrow['FirstName'] . " " . $mrow['LastName'];
                                $mcode = $mrow['MemberCode'];
                                $sql1 = sqlsrv_query($connection, "select LoanType from LoanType where LoanTypeID = '$ltid'");
                                $lrow = sqlsrv_fetch_array($sql1);
                                $LoanType = $lrow['LoanType'];
                                $sql2 = sqlsrv_query($connection, "select LoanHeading from LoanHeading where LoanHeadingID = '$lhid'");
                                $lhrow = sqlsrv_fetch_array($sql2);
                                $LoanHeading = $lhrow['LoanHeading'];
                                $sql3 = sqlsrv_query($connection, "SELECT * FROM LoanDetail WHERE 
                                                                SaveDate =(SELECT Max (SaveDate) FROM LoanDetail WHERE OfficeID='$id'
                                                                AND LoanMainID='$lid'
                                                                AND SaveDate<='$date1')
                                                                AND LoanMainID='$lid'
                                                               AND OfficeID='$id' AND (pridue>0 OR IntDue>0)");
                                $ldrow = sqlsrv_fetch_array($sql3);
                                $PriDue = $ldrow['PriDue'];
                                $IntDue = $ldrow['IntDue'];
                                $total = $PriDue + $IntDue;

                                $sql4 = sqlsrv_query($connection, "SELECT sum(loandr-loancr)par FROM LoanDetail WHERE 
                                                                LoanMainID='$lid' AND OfficeID='$id' AND savedate <= '$date1'");
                                $par = sqlsrv_fetch_array($sql4);
                                $sql5 = sqlsrv_query($connection, "SELECT count(*)instno FROM LoanDetail WHERE (pridue>0 OR IntDue>0)
                                                                AND LoanMainID='$lid' AND OfficeID='$id' AND savedate <= '$date1'");
                                $intno = sqlsrv_fetch_array($sql5);
