                ...
                <div class="box-body " >
                    ...
                    <div id="printableArea">
                        ...
                        if (isset($_POST['search'])) {
                            $q = "select OfficeID,CenterID,MemberID,MemberCode,FirstName +' '+ LastName as Mname from member where officeid = '" . $_SESSION['BranchID'] . "'  and centerid = '$cid' and Status = 'ACTIVE'";
                            $resulto = odbc_exec($connection, $q);
                        }
                        while ($rowresult = odbc_fetch_array($resulto)) {
                            ?>
                            ...
                            <table  class="table table-bordered text-center text-sm" style="font-size:11px;" >
                                <thead>

                                <caption class="text-bold text-black"><b class="box-title text-bold text-red no-print">बचत कारोवार</b><br/>नाम : <?php
                                    if (isset($_POST['search'])) {
                                        echo $rowresult['MemberCode'] . " to " . $rowresult['Mname'];
                                    } else {
                                        echo $rows['MemberCode'] . " - " . $rows['Mname'];
                                    }
                                    ?> <span class="pull-right">Date : <?php
                                    if (isset($_POST['search'])) {
                                        echo $_POST['date1'] . " to " . $_POST['date2'];
                                    } else {
                                        echo $sdate . " to " . $cdate;
                                    }
                                    ?></span></caption>
                                ...
                                <tbody class="text-sm">
                                    ...
                                    if (isset($_POST['search'])) {
                                        $date1 = $_POST['date1'];
                                        $date2 = $_POST['date2'];
                                        $query = "select savedate from savingdetail where memberid = '" . $rowresult['MemberID'] . "' and officeid = '$oid' and (dramount > 0 or cramount > 0) and 
							savedate between '$date1' and '$date2' group by savedate order by savedate";
                                        ...
                                        while ($res = odbc_fetch_array($result)) {
                                            ...
                                            <tr>
                                                ...
                                                $r1 = odbc_exec($connection, $type);
                                                while ($rows = odbc_fetch_array($r1)) {
                                                    $savingtypeid = $rows['SavingTypeID'];
                                                    $sql = "select sum(cramount)cr,sum(dramount)dr from savingdetail 
							where officeid = '$oid' and memberid = '" . $rowresult['MemberID'] . "' 
                                                        and savedate = '$savedate' and savingtypeid = '" . $rows['SavingTypeID'] . "' ";
                                                    $run1 = odbc_exec($connection, $sql);
                                                    while ($rowres = odbc_fetch_array($run1)) {
                                                        $balance = "select sum(cramount-dramount)bal from savingdetail where memberid = '" . $rowresult['MemberID'] . "' and officeid = '$oid' and savedate <= '$savedate' and savingtypeid = '" . $rows['SavingTypeID'] . "'";
                                                        $balrun = odbc_exec($connection, $balance);
                                                        $runbal = odbc_fetch_array($balrun);
                                                        ?>
                            ...
                            <table class="table table-bordered text-center text-sm" style="font-size:9px;">
                                ...
                                <tbody class="text-sm">
                                    ...
                                    if (isset($_POST['search'])) {
                                        $date1 = $_POST['date1'];
                                        $date2 = $_POST['date2'];
                                        $query = "select savedate from loandetail where memberid = '" . $rowresult['MemberID'] . "' and officeid = '$oid' and (loandr > 0 or loancr > 0 or intcr > 0) and 
						  savedate between '$date1' and '$date2' group by savedate order by savedate";
                                        $result = odbc_exec($connection, $query);
                                        while ($res = odbc_fetch_array($result)) {
                                            $savedate = $res['savedate'];
                                            ...
                                            <tr style="height:5px;" >
                                                ...
                                                while ($rows = odbc_fetch_array($r1)) {
                                                    ...
                                                    $run1 = odbc_exec($connection, $sql);
                                                    while ($rowres = odbc_fetch_array($run1)) {
                                                        $balance = "select sum(loandr-loancr)bal from loandetail where memberid = '" . $rowresult['MemberID'] . "' and officeid = '$oid' and savedate <= '$savedate' and LoanTypeID = '" . $rows['LoanTypeID'] . "'";
                                                        $balrun = odbc_exec($connection, $balance);
                                                        $runbal = odbc_fetch_array($balrun);
                                                        ?>
                                                        <td><?php echo $rowres['dr']; ?></td>
                            ...
                            <table class="table table-bordered text-center text-sm" style="font-size:9px;">
                                ...
                                <tbody>
                                    ...
                                    if (isset($_POST['search'])) {
                                        $date1 = $_POST['date1'];
                                        $date2 = $_POST['date2'];
                                        $query = "select savedate from insurancedetail where memberid = '" . $rowresult['MemberID'] . "' and officeid = '$oid' and (cramount > 0 or dramount > 0 ) and 
						  savedate between '$date1' and '$date2' group by savedate order by savedate";
                                        $result = odbc_exec($connection, $query);
                                        while ($res = odbc_fetch_array($result)) {
                                            $savedate = $res['savedate'];
