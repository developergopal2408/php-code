...
(select sum(cramount)from savingdetail where savedate='$cdate'  and c.centerid=centerid and savingtypeid=1 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')welCr,
(select sum(dramount)from savingdetail where savedate='$cdate' and c.centerid=centerid and savingtypeid=1 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')welDr,
(select sum(cramount)from savingdetail where savedate='$cdate' and c.centerid=centerid and savingtypeid=2 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')ComCr,
(select sum(dramount)from savingdetail where savedate='$cdate' and c.centerid=centerid and savingtypeid=2 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')ComDr,
(select sum(cramount)from savingdetail where savedate='$cdate' and c.centerid=centerid and savingtypeid=3 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')PerCr,
(select sum(dramount)from savingdetail where savedate='$cdate' and c.centerid=centerid and savingtypeid=3 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')PerDr,
(select sum(cramount)from savingdetail where savedate='$cdate' and c.centerid=centerid and savingtypeid=4 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')SpeCr,
(select sum(dramount)from savingdetail where savedate='$cdate' and c.centerid=centerid and savingtypeid=4 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')SpeDr,
(select sum(cramount)from savingdetail where savedate='$cdate' and c.centerid=centerid and savingtypeid=5 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')PenCr,
...
(select sum(cramount)from incomedetail where savedate='$cdate' and c.centerid=centerid and incometypeid=4 and officeid=c.officeid)Servch,
...
from centermain c where c.officeid='$pid'  group by c.centercode,c.centerid,c.officeid order by c.centercode";
                                        ...
                                        $WelCr = $WelDr = $ComDr = $ComCr = $PerCr = $PerDr = $PenCr = $EFDr = $EFCr = $PenDr = $LIc = $GenCr = $GenDr = $GenInt = $EmeCr = $EmeDr = $EmeInt = $Catt = $Passbook = $Att = $Cheq = $EduCr = $EduDr = $EduInt = $Other = $AGICr = $AGIDr = $AGIInt = $Servch = 0;
                                        ...
                                        while ($r = odbc_fetch_array($result)) {
                                            $totalreceipt = $r['welCr'] + $r['ComCr'] + $r['PerCr'] + $r['SpeCr'] + $r['PenCr'] + $r['EFCr'] + $r['LIc'] + $r['GenCr'] + $r['EmeCr'] + $r['HouCr'] + $r['EduCr'] + $r['Catt'] + $r['Passbook'] + $r['Att'] + $r['Cheq'] + $r['Other'] + $r['DseCr'] + $r['GenInt'] + $r['HouInt'] + $r['EmeInt'] + $r['EduInt'] + $r['DseInt'] + $r['AGICr'] + $r['AGIInt'] + $r['Servch'];
...
(select sum(cramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=1 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')welCr,
(select sum(dramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=1 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')welDr,
(select sum(cramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=2 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')ComCr,
(select sum(dramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=2 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')ComDr,
(select sum(cramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=3 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')PerCr,
(select sum(dramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=3 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')PerDr,
(select sum(cramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=4 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')SpeCr,
(select sum(dramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=4 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')SpeDr,
(select sum(cramount)from savingdetail where savedate BETWEEN '$date1' and '$date2' and c.centerid=centerid and savingtypeid=5 and officeid=c.officeid and TransType <> 'Interest' and RefType <> 'Interest')PenCr,
...
from centermain c where c.officeid='$ID'  group by c.centercode,c.centerid,c.officeid order by c.centercode";
                                        ...
                                        $WelCr = $WelDr = $ComDr = $ComCr = $PerCr = $PerDr = $PenCr = $EFDr = $EFCr = $PenDr = $LIc = $GenCr = $GenDr = $GenInt = $EmeCr = $EmeDr = $EmeInt = $Catt = $Passbook = $Att = $Cheq = $EduCr = $EduDr = $EduInt = $Other = $AGIDr = $AGICr = $AGIInt = $Servch = 0;
                                        $DseCr = $DseDr = $DseInt = $Cheq = $SpeCr = $SpeDr = $HouCr = $HouDr = $HouInt = $treceipt = $tdisbursed = $twithdraw = $cashsw = $netamt = 0;
